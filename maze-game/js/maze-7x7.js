...
export class maze7x7 extends Phaser.Scene
{
    constructor(){
    ...
    create() {
        ...
        desert = this.map.createLayer('desert', [mapPack]);
    ...
    //     if (!star.collected && Phaser.Geom.Intersects.RectangleToRectangle(player.getBounds(), new Phaser.Geom.Rectangle(star.x, star.y, 27, 32))) {
    //         star.collected = true;
    //         star.destroy();
    //         nStars++;
    //         // 更新界面左上角的金币数量显示
    //         timeText.setText('星星：' + nStars);
    //         // 添加一个Tween动画来让金币飞入左上角的金币数量显示处
    //         this.tweens.add({
    //             targets: star,
    //             x: timeText.x + 20/zoom,
    //             y: timeText.y + 20/zoom,
    //             duration: 1000,
    ...
    // addNewStar() {
    //     let starCenter = Phaser.Math.RND.pick(starCoordinates);
    //     const index = starCoordinates.findIndex((coordinate) => coordinate.x === starCenter.x && coordinate.y === starCenter.y);
    //     if (index !== -1) {
    //         starCoordinates.splice(index, 1);
    //     }

    //     this.star = this.add.image(starCenter.x, starCenter.y, 'star');
    //     starCoordinates
    //     this.tweens.add({
    //         targets: this.star,
    //         angle: 90,
...
var trialStart = function(scene) {
    ...
    timeText = scene.add.text(18, 18, "观察时间：5", {
            fontSize: '40px',
